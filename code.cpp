...
void crypt(u8 *src)
{
	...
	for (i = 0; i < 4; i++)
	{
		wx[i] += *((u32 *)(eKey + (IK0 + i * 4)));
	}

	for (i = 0; i < ROUND; i++)
	{
		F(wx[2], wx[3], (u32 *)&eKey[EK0 + (i * 16)], &tmp[0], &tmp[1]);
		tmp[0] ^= wx[0];
		tmp[1] ^= wx[1];
		wx[0] = wx[2];
		wx[1] = wx[3];
...
static void F(u32 ida, u32 idb, u32 *k, u32 *oda, u32 *odb)
{
	u32   wx0, wx1, wk0, wk1, tmp;
	wx0 = ida + k[0];
	wx1 = idb + k[2];
	wk0 = idb + k[1];
	wk1 = ida + k[3];
...
void WRITE(char STRING[BLOCK], char * DESTANATION){
	FILE * f;
	f = fopen(DESTANATION, "a");
...
void READ(char * DESTANATION, char * FINAL, char * DESTANATION2) {
	int SIZE;
	char STRING[BLOCK];
	FILE * f;
